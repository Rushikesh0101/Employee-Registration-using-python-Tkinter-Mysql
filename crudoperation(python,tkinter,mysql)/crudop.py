...
def Add():
    ...
    mysqldb = mysql.connector.connect(host="localhost", user="root", password="root", auth_plugin="mysql_native_password", database="payroll")
    mycursor = mysqldb.cursor()

    try:
        sql = "INSERT INTO registration (id,empname,mobile,salary) " \
              "VALUES (%s, %s, %s, %s)"
...
def update():
    ...
    try:
        sql = "Update registration set empname = %s, mobile = %s, salary = %s " \
              "where id = %s"
        val = (studname, coursename, feee, studid)
        mycursor.execute(sql, val)
        mysqldb.commit()
        # lastid = mycursor.lastrowid
        messagebox.showinfo("information", "Record Updatedd successfully...")
...
def delete():
    ...
    try:
        ...
        mysqldb.commit()
        # lastid = mycursor.lastrowid
        messagebox.showinfo("information", "record deletee successfully...")
...
def select():
    if (e1.get() == ""):
        messagebox.showinfo("information", "ID id needed for selection")
        e1.focus_set()
    else:
        mysqldb = mysql.connector.connect(host="localhost", user="root", password="root", auth_plugin="mysql_native_password", database="payroll")
        mycursor = mysqldb.cursor()
        mycursor.execute("select * from registration where id = '" + e1.get() + "'")
...
Button(root, text="Update", command=update, height=3, width=13)\
    .place(x=140, y=130)
Button(root, text="Delete", command=delete, height=3, width=13)\
    .place(x=250, y=130)
Button(root, text="Select", command=select , height=3, width=13)\
    .place(x=360, y=130)
